...
class BookSpectacle extends Component {
  ...
  updateSeat(seatName, zoneId) {
    ...
    spect.seat = seatName;
    spect.zoneId = zoneId;
    spect.price = this.state.pricesList.filter(
      zn => zn.zone.id === zoneId && zn.typ.title === spect.type
    )[0].price;
  ...
  render() {
    ...
    const canProceedToPay =
      ...
      allPaxesHaveSeats(this.state.spectatorsList) === true &&
      emptyContactDetails === false &&
      emptyPayDetails === false;

    return (
      <main>
        <section id="zonePrices">
          {this.state.fetched && <ZonePricing zones={this.state.pricesList} />}
          {!this.state.fetched && <CogWheel />}
        </section>
        ...
        <section>
          ...
          {canProceedToPay === true && (
            <div className="row paymentRow">
              <div className="col-4 offset-4">
                <button className="btn btn-success" onClick={this.pay}>
                  Complete Payment!
                </button>
              </div>
            </div>
          ...
          {emptyContactDetails && <Error message="Empty Contact Details" />}

          {emptyPayDetails && <Error message="Empty Payment Details" />}
